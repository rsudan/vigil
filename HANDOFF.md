...
- The room reader counts matching terms rather than weighting them. Two ordinary bureaucratic words admit a sentence, and one rare, room-specific word ("absorption", "successor", "threshold") does not, so the single most load-bearing sentence in a strategy can be missed while a duller one is quoted. Weighting by inverse document frequency is the fix; it was left out of this change because it alters what every room quotes and needs its own before-and-after against a real document. Room 8 on the youth strategy is the standing example: it reads as nothing quotable while p. 36 carries an explicit numbered Risks list, because that list uses none of the room's words.
