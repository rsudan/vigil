...
npm test                         # 120 unit tests (queue, validity, rooms, reading the document, extraction windows, retrieval, crypto, rate limits)
...
- Room 9's lens says a research run must leave the jurisdiction out of its peer query and accept sources about other jurisdictions. Today's per-room search (`searchRoom`) does neither: its prompt rejects a piece about another country in every room. PR C replaces it.
