...
import { IngestForm } from "@/components/strategy-workspace";
    ...
    <div className="space-y-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Each workspace watches one living document.</p>
        </div>
      ...
      {list.data?.length ? (
              ...
              <Card>
                <CardBody className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    <Link
                      to="/app/$strategyId"
                      params={{ strategyId: String(s.id) }}
                      className="font-serif text-xl hover:underline"
                    >
                      {s.title}
                    </Link>
                    <p className="mt-1 text-sm text-muted-foreground">{s.domain || "No domain"} · delivery {s.delivery_rag}</p>
                  </div>
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() =>
                      deleteStrategy({ data: { id: s.id } }).then(() =>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Load the Romania disaster-risk sample (12 load-bearing assumptions from the 2024–2035 SNRRD), or
              upload a PDF, Word, spreadsheet, or text file.
            </CardDescription>
          </CardHeader>
        </Card>
      )}
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-h-[90vh] w-[min(42rem,calc(100%-1.5rem))] overflow-y-auto">
          ...
          <DialogDescription>
            ...
            couple of hundred characters, Vigil will extract the monitoring architecture. Otherwise you get a
            blank workspace to fill by hand.
          </DialogDescription>
          <form
          ...
          >
            ...
            <Input
              ...
              placeholder="Disaster risk reduction, digital, climate…"
            />
            ...
            <Textarea
              ...
              placeholder="One paragraph on what success looks like."
            />
            ...
            <Button type="submit" disabled={!title.trim() || create.isPending}>
              {create.isPending
                ...
                : body.trim().length >= 200
                  ? "Create and extract architecture"
      ...
      <Dialog open={ingest} onOpenChange={setIngest}>
        <DialogContent className="max-h-[90vh] overflow-y-auto">
          ...
          <DialogDescription>
            PDF, Word, spreadsheet, or text — or a public URL. The whole file is stored in chunks. Extraction
            uses the model pair on the Keys page.
          </DialogDescription>
          <div className="mt-4">
            <IngestForm
              onCreated={(id) => {
